...
def calculate_development_length(points: list[dict[str, float | str]]) -> float:
    """
    ...
    Args:
        points (list[dict[str, float | str]]): List of point dictionaries, each containing:
            - point: Point identifier (e.g., "P0")
            - x: X coordinate as float
            - y: Y coordinate as float
            - z: Z coordinate as float
            - r (optional): Radius at this point as float
    ...
    try:
        ...
        for i in range(points_count - 1):
            ...
            # Extract coordinates and ensure they are float
            # Extract coordinates with type checking
            current_point = (float(str(current.get('x', 0))), float(str(current.get('y', 0))), float(str(current.get('z', 0))))
            next_point_vector = (float(str(next_point.get('x', 0))), float(str(next_point.get('y', 0))), float(str(next_point.get('z', 0))))
            ...
            if i > 0 and i < points_count - 1 and current.get('r') is not None:
                try:
                    # Get previous point for bend calculation
                    prev = points[i-1]
                    prev_point = (float(prev['x']), float(prev['y']), float(prev['z']))
                    ...
                    if theta > 0:
                        ...
                        R = float(current['r']) if current['r'] is not None else 0.0
                    ...
                    point_id = current.get('point', f'P{i}')
                    logger.warning(f"Error processing bend at point {point_id}: {e}")
                    continue
